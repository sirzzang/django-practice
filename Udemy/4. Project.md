...
## Django Project
 ...
 Docker configuration을 이용해서 Django Project 시작

* docker-compose: Django dependency가 있는 Docker 이미지 → create project files for the app



<br>

* run docker compose `docker-compose run [서비스명] [커맨드]`
  ...
  * 서비스 이름
  * Linux 컨테이너에서 실행할 커맨드(`sh -c` 없어도 되지만 있는 게 더 깔끔)
    * `sh`: shell script
    * `-c`: command
    * `django-admin.py startproject app .`: 
      * `django-admin.py`: Django admin management command when installing Django
      * `startproject`: command to [start a new project](https://docs.djangoproject.com/en/2.1/ref/django-admin/#startproject)
      * `.`: start project in current location. 처음에는 base가 Dockerfile의 last workdir

![image-20210325154700294](images/image-20210325154700294.png)



<br>

 커맨드를 실행하고 나면 `app`이라는 폴더가 생성된다!

![image-20210325154736815](images/image-20210325154736815.png)
